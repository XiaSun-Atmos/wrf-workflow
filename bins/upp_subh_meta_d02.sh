...
set -x
# module use ${UPP_DIR}/modulefiles
# module load jet
...
export TOP_DIR=/home/Xia.Sun/lfs5_rtwbl/wfip3_cases/run_dir
...
# Set run command: 
mkdir ${DOMAINPATH}/postprd_${startdate}_F${fhr}_${startmin}_d02
cd ${DOMAINPATH}/postprd_${startdate}_F${fhr}_${startmin}_d02 && mkdir parm && cd parm
cp -r ${UPP_HOME}/parm/* ${DOMAINPATH}/postprd_${startdate}_F${fhr}_${startmin}_d02/parm
...
if [ ! -d ${DOMAINPATH}/postprd_${startdate}_F${fhr}_${startmin}_d02 ]; then
...
cd ${DOMAINPATH}/postprd_${startdate}_F${fhr}_${startmin}_d02
err1=$?
...
if [ ${model} == "GFS" ]; then
   ...
   inFileName=${modelDataPath}/wrfout_d02_${timestr}
   flxFileName=${modelDataPath}/wrfout_d02_${timestr}
fi
...
if [[ ${err1} -ne 0 || ${err2} -ne 0 ]]; then
    ...
    echo 'UPP SUCCEEDED'
# Add Subfix d02 to each GRIB file
    for file in ./*; do
          if [ -f "$file" ]; then
              filename=$(basename -- "$file")
              new_filename="${filename}_d02"
    ...
    rm -rf ${DOMAINPATH}/postprd_${initdate}_F${fhr}_${startmin}_d02    
fi
