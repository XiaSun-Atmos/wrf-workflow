...
def atmo_meas2hr_upto(Din,Dqc,Dtime,Dhr_oi,AVGmins=60,func='mean'):
    ...
    for hod in range(0,len(Dhr_oi)):
        ...
        if Dqc and len(Dqc)>0:
            AVGper = np.where(np.logical_and((np.logical_and(Dtime >= Dhr_oi[hod] - AVGsecs,\
                 Dtime < Dhr_oi[hod])),Dqc == 0))[0]
        else:
            AVGper = np.where(np.logical_and(Dtime >= Dhr_oi[hod] - AVGsecs,Dtime < Dhr_oi[hod]))[0]
        #######################################################################
