#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""The Purpose of this Function is to Ingest Spatial Measurement Data (e.g. 
...
def atmo_meas2grid(MEASvar,MEASgrd,MDLgrd,AVGmins=60):
    AVGsecs=AVGmins*60  # convert to seconds
    """ Define Function Input
    ###########################################################################
    MEASvar = Gridded Spatial Data 
    ...
    MEASvar_mu = np.ones((len(MDLhgt),len(MDLtme)))*np.nan
    
    ###########################################################################
    for tod in range(0,len(MDLtme)): #for each time of day
        """ Define Relevant Time Averaging Period """
        TMEind_ref = np.logical_and(MEASt_mesh > MDLtme[tod] - AVGsecs/2,MEASt_mesh <= MDLtme[tod] + AVGsecs/2)
        ...
        for hog in range(0,len(MDLhgt)): #for each height of grid
            ...
            with warnings.catch_warnings():
                ...
                MEASin_grd = MEASvar[np.logical_and(TMEind_ref,HGTind_ref)];
                ###############################################################
                if len(MEASin_grd)>0: #require at least one observation
                    ...
                    if True:
                        MEASvar_mu[hog,tod] = np.nanmean(MEASvar[np.logical_and(TMEind_ref,HGTind_ref)])
