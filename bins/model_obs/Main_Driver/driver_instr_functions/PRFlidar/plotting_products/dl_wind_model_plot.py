        ...
        for ini in range(0,len(INIpos)):
            ...
            if INIpos[ini] in MDLini:
                ...
                if len(MDLtime) > FXlen:
                    ...
                    WSmin_ext,WSmax_ext,WStick_ext = atmo_sptl_ylim((\
                                    MDLws_ext[ext_mdl_oi]),plt_param.WSrnd_bse,plt_param.TICKint)    
                    #WDmin_ext,WDmax_ext,WDtick_ext = wd_sptl_ylim((\
                    ...
                    ###############################################################
                    # pass through array indices only of interest here 
                    WSmin,WSmax,WStick = atmo_sptl_ylim((MDLws[mdl_oi]),\
                                            plt_param.WSrnd_bse,plt_param.TICKint)    
                    #WDmin,WDmax,WDtick = wd_sptl_ylim((MDLwd[mdl_oi]));
                    ...
                    ########################################################### """
                    #cmap_wd = wd_cmap(WDmin,WDmax)
                    #cmap_wd_ext = wd_cmap(WDmin_ext,WDmax_ext)
                ...
                with warnings.catch_warnings():
                    ...
                    ws_dl = dl_ax.imshow(MDLws*np.nan,extent=MDLgrid,vmin=WSmin,vmax=WSmax,\
                ...
                dl_ax.text(np.nanmean([Xmin,Xmax]),np.nanmean([HGTmin,MAXhgt]),'Measurement Data Not Available',\
                ...
                ###################################################################
                cbar = ws_fig.colorbar(bias_ws,ax=bias_ax,ticks = np.arange(-3,3.1,1),
                    pad=plt_param.Cbar_pad,aspect = plt_param.CBaspect)
                cbar.ax.tick_params(labelsize=plt_param.Ctck_fs)
                ...
                FILout = os.path.join(IMGpth_meas,FILpfx+MDLname+'_WS_'+Sid+'.'+DATE+'_INI'+INIpos[ini]+'.png')
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=UserWarning)
                    wd_dl = dl_ax.imshow(MDLwd*np.nan,extent=MDLgrid,vmin=WDmin,vmax=WDmax,\
                ...
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=UserWarning)
                    bias_wd = bias_ax.imshow(MDLwd*np.nan,extent=MDLgrid,vmin=-90,vmax=90,\
                                     origin='lower',aspect='auto',cmap = cmap_bias);   
                bias_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                ###################################################################
                cbar = wd_fig.colorbar(bias_wd,ax=bias_ax,ticks = np.arange(-90,90.1,45),pad=plt_param.Cbar_pad,aspect = plt_param.CBaspect)
                cbar.ax.tick_params(labelsize=plt_param.Ctck_fs)
                ...
                ###################################################################          
                #Save the Developed Figure
                FILout = os.path.join(IMGpth_meas,FILpfx+MDLname+'_WD_'+Sid+'.'+DATE+'_INI'+INIpos[ini]+'.png')
                ...
                if 'MDLtime_ext' in locals():
                    ...
                    ws_fig.suptitle('DL WS',x = plt_param.DLmeas_x_title,\
                            ...
                            fontsize=plt_param.TITLEfs);
                    ###############################################################
                    ws_dl = dl_ax.imshow(MDLws_ext*np.nan,extent=MDLgrid_ext,vmin=WSmin_ext,vmax=WSmax_ext,\
                                         origin='lower',aspect='auto',cmap = cmap_ws);   
                    dl_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5);
                    ###############################################################
                    ...
                    ###############################################################
                    # Denote that Measurement Data was Not Available
                    dl_ax.text(np.nanmean([Xmin_ext,Xmax_ext]),np.nanmean([HGTmin,MAXhgt]),'Measurement Data Not Available',\
                             horizontalalignment='center',verticalalignment='bottom',color='red',\
                             fontweight='semibold',fontsize=plt_param.TITLEfs);
                    ###################################################################
                    dl_ax.set_ylabel('Height, m AGL',{'fontsize': plt_param.LABELfs});
                    ...
                    wd_fig.suptitle('DL WD',x = plt_param.DLmeas_x_title,\
                            ...
                            fontsize=plt_param.TITLEfs);
                    ###############################################################
                    wd_dl = dl_ax.imshow(MDLwd_ext*np.nan,extent=MDLgrid_ext,vmin=WDmin_ext,\
                                         vmax=WDmax_ext,origin='lower',aspect='auto',cmap = cmap_wd);   
                    dl_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5);
                    ###############################################################
                    ...
                    # Plot the Model Wind Direction Bias
                    ##########################################
                    bias_wd = bias_ax.imshow(MDLwd_ext*np.nan,extent=MDLgrid_ext,vmin=-90,vmax=90,\
                                     origin='lower',aspect='auto', cmap = cmap_bias);   
                    bias_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                    ###############################################################
                    cbar = wd_fig.colorbar(bias_wd,ax=bias_ax,ticks = np.arange(-90,90.1,45),pad=plt_param.Cbar_pad,aspect = plt_param.CBaspect)
