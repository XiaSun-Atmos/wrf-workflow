...
def psl_RADplot(MEASin,MDLin,OUTinfo):
    DATE = OUTinfo[3];
        ...
        for ini in range(0,len(INIpos)):
            ...
            if INIpos[ini] in MDLini:
                ...
                if len(MDLtime) > FXlen: #indicating forecast hours exceed standard forecast length
                    ...
                    MEASsolar_ext_mu = atmo_meas2hr_end(MEASsolar,MEASsolar_qc,MEAStime,MDLtime_ext, AVGmins=AVGper);
                    MEASnet_rad_ext_mu = atmo_meas2hr_end(MEASnet_rad,MEASnet_rad_qc,MEAStime,MDLtime_ext, AVGmins=AVGper);
                    ###############################################################
                    SRADmin_ext,SRADmax_ext,SRADtick_ext = atmo_ylim((MEASsolar[ext_meas_oi],\
                                                   MDLsw_dwn_ext),plt_param.SRADrnd_bse,plt_param.TICKint)
                    NRADmin_ext,NRADmax_ext,NRADtick_ext = atmo_ylim((MEASnet_rad[ext_meas_oi],\
                                                   MDLnet_rad_ext),plt_param.SRADrnd_bse,plt_param.TICKint)
                    ###############################################################
                    ...
                    SRADmin,SRADmax,SRADtick = atmo_ylim((MEASsolar[meas_oi],\
                                                   MDLsw_dwn),plt_param.SRADrnd_bse,plt_param.TICKint)
                    NRADmin,NRADmax,NRADtick = atmo_ylim((MEASnet_rad[meas_oi],\
                                                   MDLnet_rad),plt_param.SRADrnd_bse,plt_param.TICKint)
                    ###############################################################
                    #Define Xtick Information (Push Out to Maximum Forecast Time)
                ...
                if (np.isnan(MEASsolar_mu).all()):
                    bias_ax.plot(MDLtime,MDLtime*np.nan,'^-',color='forestgreen',markersize=plt_param.PLTmsize,\
                        linewidth=plt_param.PLTlwidth)
                else:
                    SRADbias = MDLsw_dwn - MEASsolar_mu
                    RMSEsw_dwn,_ = atmo_bias(SRADbias);
                    bias_ax.plot(MDLtime,SRADbias,'^-',color='forestgreen',markersize=plt_param.PLTmsize,\
                        linewidth=plt_param.PLTlwidth,label='Solar Radiation (RMSE: ' + str(RMSEsw_dwn)+')')
                if (np.isnan(MEASnet_rad_mu).all()):
                    bias_ax.plot(MDLtime,MDLtime*np.nan,'^-',color='dodgerblue',markersize=plt_param.PLTmsize,\
                        linewidth=plt_param.PLTlwidth)
                else:
                    NRADbias = MDLnet_rad - MEASnet_rad_mu
                    RMSEnet_rad,_ = atmo_bias(NRADbias);
                    bias_ax.plot(MDLtime,NRADbias,'^-',color='dodgerblue',markersize=plt_param.PLTmsize,\
                        linewidth=plt_param.PLTlwidth,label='Net Radiation (RMSE: ' + str(RMSEnet_rad)+')')
                ###################################################################
                bias_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                ###################################################################
                if (not np.isnan(MEASsolar_mu).all()):
                ...
                ####################################################################
                #Define Bounds of bias bases on Solar - Model Bias to Determine Y-Bounds
                if (np.isnan(MEASsolar_mu).all()):
                    SRADb_min=-100;SRADb_max=100;SRADb_tick=np.arange(SRADb_min,SRADb_max+1,50)
                else:
                    SRADb_min,SRADb_max,SRADb_tick = atmo_blim((SRADbias,NRADbias),plt_param.SRADbias_bse,plt_param.TICKint)
                bias_ax.set_ylim(SRADb_min,SRADb_max); bias_ax.set_yticks(SRADb_tick);
                ...
                if 'MDLtime_ext' in locals():
                    ...
                    # Plot the Shortwave Downwelling Time Histories
                    ###############################################
                    sw_ax.plot(MDLtime_ext,MEASsolar_ext_mu,'s-',color='gray',markersize=plt_param.PLTmsize,label='Obs (1Hr Avg)')
                    sw_ax.plot(MEAStime,MEASsolar,'-',color='black',linewidth=plt_param.PLTlwidth,markersize=plt_param.PLTmsize,label='Obs')
                    ...
                    # Plot the Net Radiation Time Histories
                    ###############################################
                    net_ax.plot(MDLtime_ext,MEASnet_rad_ext_mu,'s-',color='gray',markersize=plt_param.PLTmsize,label='Obs (1Hr Avg)')
                    net_ax.plot(MEAStime,MEASnet_rad,'-',color='black',linewidth=plt_param.PLTlwidth,markersize=plt_param.PLTmsize,label='Obs')
                    ...
                    ###################################################################
                    # Define the Bias (Observations - Measurements)
                    if (np.isnan(MEASsolar_ext_mu).all()):
                        bias_ax.plot(MDLtime_ext,MDLtime_ext*np.nan,'^-',color='forestgreen',markersize=plt_param.PLTmsize,\
                            linewidth=plt_param.PLTlwidth)
                    else:
                        SRADbias = MDLsw_dwn_ext - MEASsolar_ext_mu
                        RMSEsw_dwn,_ = atmo_bias(SRADbias);
                        bias_ax.plot(MDLtime_ext,SRADbias,'^-',color='forestgreen',markersize=plt_param.PLTmsize,\
                            linewidth=plt_param.PLTlwidth,label='Solar Radiation (RMSE: ' + str(RMSEsw_dwn)+')')
                    if (np.isnan(MEASnet_rad_ext_mu).all()):
                        bias_ax.plot(MDLtime_ext,MDLtime_ext*np.nan,'^-',color='dodgerblue',markersize=plt_param.PLTmsize,\
                            linewidth=plt_param.PLTlwidth)
                    else:
                        NRADbias = MDLnet_rad_ext - MEASnet_rad_ext_mu
                        RMSEnet_rad,_ = atmo_bias(NRADbias);
                        bias_ax.plot(MDLtime_ext,NRADbias,'^-',color='dodgerblue',markersize=plt_param.PLTmsize,\
                            linewidth=plt_param.PLTlwidth,label='Net Radiation (RMSE: ' + str(RMSEnet_rad)+')')
                    ###################################################################
                    bias_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                    ###################################################################
                ...
                sw_ax.plot(MDLtime,MEASsolar_mu,'s-',color='gray',markersize=plt_param.PLTmsize,label='Obs (1Hr Avg)')
                sw_ax.plot(MEAStime,MEASsolar,'-',color='black',linewidth=plt_param.PLTlwidth,markersize=plt_param.PLTmsize,label='Obs')
                sw_ax.plot(MDLtime,MDLtime*np.nan,'o-',color='red',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label=MDLname+MDLstr)
                sw_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                ####################################################################
                handles, labels = sw_ax.get_legend_handles_labels()
                order = [1,0,2]
                ...
                net_ax.plot(MDLtime,MEASnet_rad_mu,'s-',color='gray',markersize=plt_param.PLTmsize,label='Obs (1Hr Avg)')
                net_ax.plot(MEAStime,MEASnet_rad,'-',color='black',linewidth=plt_param.PLTlwidth,markersize=plt_param.PLTmsize,label='Obs')
                net_ax.plot(MDLtime,MDLtime*np.nan,'o-',color='red',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label=MDLname+MDLstr)
                net_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                ####################################################################
                net_ax.set_ylim(NRADmin,NRADmax); net_ax.set_yticks(NRADtick);
                net_ax.tick_params(axis="y", labelsize=plt_param.TICKfs)
                ...
                bias_ax.plot([Xmin,Xmax],[0,0],color='black',linewidth=1.5)
                ###################################################################
                bias_ax.plot(MDLtime,MDLtime*np.nan,'^-',color='forestgreen',markersize=4,\
                                 linewidth=plt_param.PLTlwidth,label='Solar Radiation (RMSE: N/A)')
                bias_ax.plot(MDLtime,MDLtime*np.nan,'^-',color='dodgerblue',markersize=4,\
                ...
                bias_ax.set_ylim(-100,100); bias_ax.set_yticks(np.arange(-100,100.1,40));
                bias_ax.tick_params(axis="y", labelsize=plt_param.TICKfs)
                ###################################################################
                bias_ax.set_ylabel('Model Error,\n W $\mathrm{m}^{-2}$',{'fontsize': plt_param.LABELfs});    
                bias_ax.yaxis.set_label_coords(plt_param.Ylab_pad[0],plt_param.Ylab_pad[1]);
                ...
                #######################################################################
                #Save the Developed Figure
                FILout = os.path.join(IMGpth_model,FILpfx+MDLname+'_SRAD_'+Sid+'.'+DATE+'_INI'+INIpos[ini]+'.png')
                rad_fig.savefig(FILout,dpi=300); plt.close('all');
                ###################################################################
