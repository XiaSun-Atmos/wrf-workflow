...
def gmlceil_plot(MEASin,MDLin,OUTinfo):
    DATE = OUTinfo[3];
        ...
        for ini in range(0,len(INIpos)):
            ...
            if INIpos[ini] in MDLini:
                ...
                if len(MDLtime) > FXlen: #indicating forecast hours exceed standard forecast length
                    ...
                    MEAScbh_ext_mu = atmo_meas2hr_mid(MEAScbh,None,MEAStime,MDLtime_ext,AVGmins=AVGper,func='median');
                    ###############################################################
                    CBHmin_ext,CBHmax_ext,CBHtick_ext = atmo_bound_lim((MEAScbh[ext_meas_oi],\
                                                   MDLcbh_ext),plt_param.CBHrnd_bse,plt_param.CBHmin,plt_param.CBHmax,\
                    ...
                    CBHmin,CBHmax,CBHtick = atmo_bound_lim((MEAScbh[meas_oi],\
                                                   MDLcbh),plt_param.CBHrnd_bse,plt_param.CBHmin,plt_param.CBHmax,\
                                                   plt_param.CBHrnd_bse)
                    ###############################################################
                    #Define Xtick Information (Push Out to Maximum Forecast Time)
                ...
                if (np.isnan(MDLcbh).all()):
                    bias_ax.plot(MDLtime,MDLtime*np.nan,'^-',color='red',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label='Mdl - Obs')
                else:
                    CBHbias = MDLcbh - MEAScbh_mu
                    bias_ax.plot(MDLtime,CBHbias,'^-',color='red',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label='Mdl - Obs')
                ...
                ####################################################################
                #Define Bounds of Cloud Base Height  - Model Bias to Determine Y-Bounds
                if (np.isnan(MDLcbh).all()):
                    CBHb_min=-300; CBHb_max = 300; CBHb_tick = np.arange(CBHb_min, CBHb_max+1, 100);
                else:
                    CBHb_min,CBHb_max,CBHb_tick = atmo_bound_lim((CBHbias),plt_param.CBHbias_bse,plt_param.CBHbmin,\
                                                     plt_param.CBHbmax,plt_param.CBHbias_bse)
                bias_ax.set_ylim(CBHb_min,CBHb_max); bias_ax.set_yticks(CBHb_tick);
                ...
                if (np.isnan(MDLcbh).all()):
                    ...
                    RMSEcbh,BIAScbh = atmo_bias(CBHbias)
                    #insert data statistics as title
                    BT = bias_ax.set_title('RMSE: ' + str(RMSEcbh)+' mm | Mean Bias: '+\
                                          str(BIAScbh)+' mm',{'fontsize': plt_param.TITLEfs,'horizontalalignment':'left'},pad=2.5) 
                ...
                FILout = os.path.join(IMGpth,FILpfx+MDLname+'_CBH_'+Sid+'.'+DATE+'_INI'+INIpos[ini]+'.png')
                ...
                if 'MDLtime_ext' in locals():
                    ...
                    #####################################
                    #Observations 
                    cbh_ax.plot(MDLtime_ext,MEAScbh_ext_mu,'s',color='gray',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label='Obs ('+AVGlabel+' Avg)')
                    #cbh_ax.plot(MEAStime,MEAScbh, color='black',linewidth=plt_param.PLTlwidth,label='Obs')
                    cbh_ax.plot(MEAStime,MEAScbh,'.',color='black',markersize=1,linestyle='None',label='Obs')
                    ...
                    if (np.isnan(MDLcbh).all()):
                        ...
                        CBHbias = MDLcbh_ext - MEAScbh_ext_mu
                        bias_ax.plot(MDLtime_ext,CBHbias,'^-',color='red',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label='Mdl - Obs')
                    bias_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                    ###############################################################
                    bias_ax.legend(loc=1,fontsize=plt_param.LEGfs,bbox_to_anchor = plt_param.LEGpad_dual,\
                                 ncol = 1,frameon = False);    
                ...
                #####################################
                #Observations 
                cbh_ax.plot(MDLtime,MEAScbh_mu,'s',color='gray',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label='Obs ('+AVGlabel+' Avg)')
                #cbh_ax.plot(MEAStime,MEAScbh, color='black',linewidth=plt_param.PLTlwidth,label='Obs')
                cbh_ax.plot(MEAStime,MEAScbh,'.',color='black',markersize=1,linestyle='None',label='Obs')
                cbh_ax.plot(MDLtime,MDLtime*np.nan,'o-',color='red',markersize=plt_param.PLTmsize,linewidth=plt_param.PLTlwidth,label=MDLname+MDLstr)
                cbh_ax.grid('both',color='k',linestyle='-',linewidth=0.1,alpha=0.5)
                ####################################################################
                handles, labels = cbh_ax.get_legend_handles_labels()
                order = [1,0,2]
                ...
                """ ###############################################################
                # Plot the Model Cloud Base Height iases
                ############################################################### """
                # initate 0/0 line
                bias_ax.plot([Xmin,Xmax],[0,0],color='black',linewidth=plt_param.PLTlwidth)
                ###################################################################
                ...
                bias_ax.set_ylim(-300,300); bias_ax.set_yticks(np.arange(-300,300.1,100));
                bias_ax.tick_params(axis="y", labelsize=plt_param.TICKfs)
                ###################################################################
                bias_ax.set_ylabel('Model Error, m',{'fontsize': plt_param.LABELfs});    
                bias_ax.yaxis.set_label_coords(plt_param.Ylab_pad[0],plt_param.Ylab_pad[1]);
