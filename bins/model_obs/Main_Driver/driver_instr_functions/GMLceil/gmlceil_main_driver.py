...
# -*- coding: utf-8 -*-
"""
The Purpose of this Script is to Produce GML Ceilometer Plots
###############################################################################
- Edits:
...
#########################################
from gmlceil_main_ingest import gmlceil_ingest
from esrl_main_ingest import esrl_ingest
#########################################
from gmlceil_plot import gmlceil_plot
from gmlceil_meas_plot import gmlceil_meas
from gmlceil_model_plot import gmlceil_model
#########################################
import station_param_list as station_prm
import forecast_param_list as fxcst_param
#########################################
from pslutils import get_gmlceil_info

def gmlceil_driver(Soi,OUTdirs,anlyper,overwrite=False):

    """ Denote Analysis for this Datastream Channel is Starting
    ####################################################################### """
    print('----> GML Ceilometer Data Analysis Starting (' + datetime.now().strftime('%Y-%b-%d -- %H:%M:%S MT') + ')')
    ...
    ####################################################################### """
    print('----> Analyzing GML Ceilometer Data: (' + strt_date + ' -- ' + end_date + ')')
    ...
    for Sid in Soi:
        ...
        ceil_Sid=getattr(station_prm,Sid+'name')  #normal station id
        ceil_Pid=getattr(station_prm,Sid+'projID') # project station id
        MEASout,Pmeas_oi,dateindex,suffix = get_gmlceil_info(station_prm.EXPname, \
               meas_dir,ceil_Sid,ceil_Pid)

        """ Define Image Output Directory """
        IMGout = os.path.join(img_dir,Sid+'out/'+ceil_name+'/')
        ...
        for MODEL_name in MODEL_names:
            ...
            GMLceildata = [fname for fname in os.listdir(MEASout) if ((fname.startswith(Pmeas_oi)) & (fname.endswith(suffix)))]
            ...
            for doi in MODEL_date2proc:
                ...
                print('----> Analyzing GML Ceilometer Data: ' + MODEL_name + ' v ' + Sid + ' (' + doi +')')
                ###################################################################
                """ Ingest and Process Measurement Data """
                GMLceilloc, GMLceilxy, GMLceilhgt = gmlceil_ingest(MEASout,GMLceildata,doi,suffix=suffix)
                ###################################################################
                # Load in Data from doi+1
                doi_plus = (date(int(doi[0:4]),int(doi[4:6]),int(doi[6:8])) + timedelta(days=1)).strftime('%Y%m%d')
                _, GMLceilxy_plus, GMLceilhgt_plus = gmlceil_ingest(MEASout,GMLceildata,doi_plus,suffix=suffix)
                ###################################################################
                # Load in Data from doi-1
                doi_minus = (date(int(doi[0:4]),int(doi[4:6]),int(doi[6:8])) - timedelta(days=1)).strftime('%Y%m%d')
                _, GMLceilxy_minus, GMLceilhgt_minus = gmlceil_ingest(MEASout,GMLceildata,doi_minus,suffix=suffix)
                ###################################################################
                """ Incorporate doi + 2 for Extended Forecasts """
                doi_ext = (date(int(doi[0:4]),int(doi[4:6]),int(doi[6:8])) + timedelta(days=2)).strftime('%Y%m%d')
                _, GMLceilxy_ext, GMLceilhgt_ext = gmlceil_ingest(MEASout,GMLceildata,doi_ext,suffix=suffix)
                ###################################################################
                ...
                """ Produce GMLceil Comparisons """
                # Define Measurement Time Input Tuple
                GMLceiltme = (GMLceilxy_minus,GMLceilxy,GMLceilxy_plus,GMLceilxy_ext)
                ###################################################################
                # Define variable tuples
                GMLceil_hgt = (GMLceilhgt_minus,GMLceilhgt,GMLceilhgt_plus,GMLceilhgt_ext);
                # Declare Measurement Input Plotting Tuples 
                GMLceil_plt = (GMLceilloc,GMLceiltme,GMLceil_hgt)
                ###################################################################
                ...
                ###################################################################
                MEASchck = len(GMLceilhgt) > 0 #require measurements for doi defined
    ...
    ####################################################################### """
    print('----> GML Ceilometer Data Analysis Complete (' + datetime.now().strftime('%Y-%b-%d -- %H:%M:%S MT') + ')')
